...
void FolderBrowserOverlay::drawEmptyMessage(void) const
{
    ...
    text.drawCentered("X to sync into this folder.", 0, 400, 142, 0.42f, COLOR_FAINT, OVERLAY_Z);
}
...
std::string FolderBrowserOverlay::bottomHints(void) const
{
    ...
    return std::string(GLYPH_A) + " Open   " + GLYPH_X + " Use this folder   " + GLYPH_B + " " + (atRoot ? "Cancel" : "Up");
}

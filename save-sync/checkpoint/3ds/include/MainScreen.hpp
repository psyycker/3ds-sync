...
private:
    ...
    // Single per-title action button, replacing the old Backup/Restore pair:
    // "Set Tag" when the selected title has no Drive sync tag yet, "Sync"
    // once it does. Sized by layoutActionButtons().
    std::unique_ptr<Clickable> buttonTagSync;
    std::unique_ptr<Clickable> buttonSend; // shares the row with buttonTagSync when wireless transfer is on (greyed unless sendable)
